...
Copyright (c) 2014 Greg Garbern (greggar)
...
{
    s_pDriverObject = (PDRIVER_OBJECT)DriverObject;
...

{
    ...
    *Events &= ~(SDHC_IS_CARD_DETECT |
    ...
    return (BOOLEAN)((*Events != 0) || (*CardChange) || (*SdioInterrupt) || (*Tuning));
}
...

{
    ...
    //
    // Mark the current request as being outstanding.
    //

    if (InterlockedExchangePointer(
        (void *volatile *) & SdhcExtension->OutstandingRequest, Request) != NULL) {
...

{
    ...
    if (InterlockedExchangePointer(
        (void *volatile *) & SdhcExtension->OutstandingRequest, 
        SdhcExtension->OutstandingRequest) == NULL) {
        return;
    }
...

{
    ...
    if (InterlockedExchangePointer(
        (void *volatile *) & SdhcExtension->OutstandingRequest, NULL) != NULL) {
        // Aborted previous request.
    }
    ...
    )
{
    if (Frequency < 100000000) {
        SdhcWriteRegisterUlong(SdhcExtension, DWCMSHC_EMMC_DLL_CTRL, 0);
        SdhcWriteRegisterUlong(SdhcExtension, DWCMSHC_EMMC_DLL_RXCLK, 0);
        SdhcWriteRegisterUlong(SdhcExtension, DWCMSHC_EMMC_DLL_TXCLK, 0);
        SdhcWriteRegisterUlong(SdhcExtension, DWCMSHC_EMMC_DLL_STRBIN,
            DWCMSHC_EMMC_DLL_STRBIN_DLYENA | DWCMSHC_EMMC_DLL_STRBIN_DELAY_NUM_SEL
            | DWCMSHC_EMMC_DLL_STRBIN_DELAY_NUM_DEFAULT << DWCMSHC_EMMC_DLL_STRBIN_DELAY_NUM_SHIFT);
        SdhcWriteRegisterUlong(SdhcExtension, DWCMSHC_EMMC_DLL_CMDOUT, 0);
...

{
    ...
    //
    // RKCP0D40:
    // The clock divider is broken on this implementation of the controller.
    // 
    ...
    Status = DwcSdhcRkSetClock(SdhcExtension, Frequency);
...

{
    ...
    //
    // RKCP0D40:
    // SDHC_PS_WRITE_PROTECT in SDHC_PRESENT_STATE seems to be always 0 (read-only).
    //
    return FALSE;
}
...

{
    ...
    CurrentEvents = InterlockedExchange((PLONG)&SdhcExtension->CurrentEvents, 0);
...

{
    ...
    Buffer = (PUCHAR) Request->Command.DmaVirtualAddress;
    Descriptor = NULL;
    ...
    if (IsCommandCompleted) {
        CurRequest = (const SDPORT_REQUEST*)
            InterlockedExchangePointer(
                (void *volatile *) & SdhcExtension->OutstandingRequest, NULL);
