...
private:
    ...
    PCCsAudioRk3xHW        m_pHW;                  // HW object
    ...
    STDMETHODIMP_(char*) GetTopology(
        _Out_ BOOLEAN* isJack
    );
    ...
    (
        ...
        _In_reads_opt_(cPropertyCount) const CSAUDIORK3X_DEVPROPERTY* pProperties,
...
NTSTATUS CsAudioRk3xIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const CSAUDIORK3X_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    RKDSP_BUS_INTERFACE RKdspInterface;
    ntStatus = WdfFdoQueryForInterface(m_WdfDevice, &GUID_RKDSP_BUS_INTERFACE, (PINTERFACE)&RKdspInterface, sizeof(RKdspInterface), 1, NULL);
    if (!NT_SUCCESS(ntStatus)) {
        DPF(D_TERSE, ("Unable to query RKdsp Interface"));
    }
    IF_FAILED_JUMP(ntStatus, Done);

    // Initialize HW.
    // 
    m_pHW = new (NonPagedPool, CSAUDIORK3X_POOLTAG)  CCsAudioRk3xHW(&RKdspInterface, m_WdfDevice);
    if (!m_pHW)
    {
        DPF(D_TERSE, ("Insufficient memory for Rockchip I2S HW"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
    IF_FAILED_JUMP(ntStatus, Done);
    ...
    {
        DPF(D_TERSE, ("Insufficient resources for Rockchip I2S HW"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
    IF_FAILED_JUMP(ntStatus, Done);

    ntStatus = m_pHW->rk3x_init();
    if (!NT_SUCCESS(ntStatus))
    {
        DPF(D_TERSE, ("Unable to in initialize Rockchip I2S"));
    }
    IF_FAILED_JUMP(ntStatus, Done);
...
CAdapterCommon::PrepareDMA(
    ...
    if (m_pHW) {
        return  m_pHW->rk3x_program_dma(deviceType, mdl, stream, byteCount);
    }
...
CAdapterCommon::StartDMA(
    ...
    if (m_pHW) {
        return m_pHW->rk3x_play(deviceType);
    }
    return STATUS_NO_SUCH_DEVICE;
}
...
CAdapterCommon::StopDMA(
    ...
    if (m_pHW) {
        return m_pHW->rk3x_stop(deviceType);
    }
    return STATUS_NO_SUCH_DEVICE;
}
...
CAdapterCommon::CurrentPosition(
    ...
    if (m_pHW) {
        return m_pHW->rk3x_current_position(deviceType, linkPos, linearPos);
    }
    return STATUS_NO_SUCH_DEVICE;
}
...
#pragma code_seg()
STDMETHODIMP_(char*)
CAdapterCommon::GetTopology(
    _Out_ BOOLEAN* isJack
) {
    if (m_pHW) {
        if (isJack)
            *isJack = m_pHW->isJack;
        return m_pHW->rkTPLG.audio_tplg;
    }
    return "";
}
...
--*/
{
    ...
    //
    ntStatus = CsAudioRk3xIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: CsAudioRk3xIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with CSAUDIORK3X.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with CSAUDIORK3X.<WaveName>.szPname in the inf's [Strings] section
